...
class VideoCell:BaseCell{
    ...
    var video :Video?{
        didSet{
            ...
            if let channelName = video?.channel?.name, let numberOfViewsAfterFormatting = numberFormatter.string(from: (video?.number_of_views)!){
                subTitleView.text = "\(channelName) : \(numberOfViewsAfterFormatting)" + "\n" + "2 Years ago"
            }
            
            //measuringTitleLabelTextHeight
            if let title = video?.title{
                let size = CGSize.init(width: frame.width - 16 - 8 - 44 - 16 , height: 1000)
